...
<!--			        --><?//echo "<pre>"; var_dump(!IsSeoDisrupting($arParams)); echo "</pre>";?>
			<? if (!substr_count($_SERVER['REQUEST_URI'], '/filter/')) : ?>
					...
					<?//КМ между H1 и подразделами?>
						<? include_once(__DIR__ . "/../km_mezhduh1_i_podrazdelami.php") ?>
					<?//КМ между H1 и подразделами?>
...
<? else: ?>
			...
			<? if  (strpos($_SERVER['REQUEST_URI'], 'PAGEN') === false):?>
										...
										<? include_once(__DIR__ . "/../seo_dlya_posadochnyh_stranic_verx.php") ?>
										<?/*SEO текст для посадочных страниц каталога по регионам ВЕРХ*/?>
				<? else: ?>
...
<? if (!substr_count($_SERVER['REQUEST_URI'], '/filter/')) : ?>
	   <? if ($iSectionsCount): ?>			
			...
			<? include_once(__DIR__ . "/../km_mezhdu_podrazdelami_i_elementami.php") ?>
			<?//КМ между подразделами и элементами?>
			 </div>
		<? endif; ?>
 <? endif; ?>
...
<? /*БЛОК */ ?>
 <? if (!$arSeoItem): ?>
		...
		<?//КМ верхние теги?>
			<? include_once(__DIR__ . "/../km_top_tag.php") ?>
			<?//КМ верхние теги?>
...
<? if ($itemsCnt): ?>
			...
			<? if ($APPLICATION->GetProperty("HIDE_ELEMENT_LIST") != "Y") {
				include_once(__DIR__ . "/../sort.php");
			} else {
				$template = 'catalog_hidden';
			} ?>
...
<?//UF_ELEMENTS (Не убирать элементы подраздела) - по умолчанию свойство в значении ДА, если галочку снимаем, то элементы раздела пропадают?>
...
<? if (!substr_count($_SERVER['REQUEST_URI'], '/filter/')) : ?>
		...
		<?//КМ нижние теги?>
			<? include_once(__DIR__ . "/../km_bottom_tag.php") ?>
			<?//КМ нижние теги?>

				
			<?//КМ после товаров?>
			<? include_once(__DIR__ . "/../km_posle_tovarov.php") ?>
			<?//КМ после товаров?>
  ...
  <? $APPLICATION->SetPageProperty("robots", "noindex, follow"); ?>
        ...
        <?endif?>
                    ...
                    <? /*Популярные категории*/ ?>
							...
							<? include_once(__DIR__ . "/../seo_dlya_posadochnyh_stranic_niz.php") ?>
							<?/*SEO текст для посадочных страниц каталога по регионам НИЗ*/?>
