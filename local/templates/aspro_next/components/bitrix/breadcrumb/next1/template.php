...
if($arResult){
	...
	for($index = 0; $index < $cnt; ++$index){
		...
		if($arItem["LINK"] <> "" && $arItem['LINK'] != GetPagePath() && $arItem['LINK']."index.php" != GetPagePath() || $arSubSections){
			$strReturn .= '<div class="bx-breadcrumb-item'.($bMobileBreadcrumbs ? ' bx-breadcrumb-item--mobile' : '').($bShowMobileArrow ? ' bx-breadcrumb-item--visible-mobile' : '').($arSubSections ? ' ' : '').($bLast ? ' cat_last' : '').'" id="bx_breadcrumb_'.$index.'" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">';
			if($arSubSections){
