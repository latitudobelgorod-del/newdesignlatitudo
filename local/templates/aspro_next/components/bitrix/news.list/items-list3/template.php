			...
			<div id="<?=$this->GetEditAreaId($arSection['ID'])?>" class="tab-pane <?=(!$si++ || !$arSection['ID'] ? 'active' : '')?>">
				...
				<div class="row items">
					<?foreach($arSection['ITEMS'] as $i => $arItem):?>
						...
						<div class="shadow col-md-<?=floor(12 / $line_element_count)?> col-sm-<?=floor(12 / round($line_element_count / 2))?> col-xs-6">
								<?if($bDetailLink):?>
										<a href="<?=$arItem['DETAIL_PAGE_URL']?>" title="<?=$arItem['NAME']?>">
									<?endif;?><div class="item clearfix" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
