...
define('HL_BLOCK_ID', 9); // ID вашего highload-блока с цветами

if (empty($arResult['ITEMS'])) {
    return;
}

// !!! ИСПРАВЛЕНИЕ ОШИБКИ: гарантируем, что SKU_PROPS всегда массив
if (!isset($arResult['SKU_PROPS']) || !is_array($arResult['SKU_PROPS'])) {
    $arResult['SKU_PROPS'] = [];
}

// Собираем все ID связанных элементов из всех элементов раздела
$allLinkedIds = [];
$linkedByItem = [];

foreach ($arResult['ITEMS'] as $item) {
    // Свойство может лежать в PROPERTIES или DISPLAY_PROPERTIES
    $prop = $item['PROPERTIES']['ASSOCIATED'] ?? $item['DISPLAY_PROPERTIES']['ASSOCIATED'] ?? null;
    
    if ($prop && !empty($prop['VALUE'])) {
        // !!! ИСПРАВЛЕНИЕ: Приводим к массиву, даже если значение одно
        $linkedIds = (array)$prop['VALUE'];
        $linkedByItem[$item['ID']] = $linkedIds;
        $allLinkedIds = array_merge($allLinkedIds, $linkedIds);
    } else {
        $linkedByItem[$item['ID']] = [];
    }
}

$allLinkedIds = array_unique($allLinkedIds);
if (empty($allLinkedIds)) {
    return;
}

// Получаем данные связанных элементов (ID, название, URL, значение свойства COLOR_MAIN_EL, PREVIEW_PICTURE)
$linkedElementsData = [];
$rsLinked = CIBlockElement::GetList(
    [],
    ['IBLOCK_ID' => $arResult['IBLOCK_ID'], 'ID' => $allLinkedIds, 'ACTIVE' => 'Y'],
    false,
    false,
    ['ID', 'NAME', 'DETAIL_PAGE_URL', 'PREVIEW_PICTURE', 'PROPERTY_COLOR_MAIN_EL']
);
while ($linked = $rsLinked->GetNextElement()) {
    $fields = $linked->GetFields();
    $props = $linked->GetProperties();
    
    // Получаем путь к картинке анонса
    $previewPicturePath = '';
    if (!empty($fields['PREVIEW_PICTURE'])) {
        $previewPicturePath = CFile::GetPath($fields['PREVIEW_PICTURE']);
    }
    
    $linkedElementsData[$fields['ID']] = [
        'ID'    => $fields['ID'],
        'NAME'  => $fields['NAME'],
        'URL'   => $fields['DETAIL_PAGE_URL'],
        'PREVIEW_PICTURE_PATH' => $previewPicturePath,
        'COLOR_XML_ID' => $props['COLOR_MAIN_EL']['VALUE'] ?? ''
    ];
}

// Собираем уникальные XML_ID цветов
$allColorXmlIds = array_unique(array_column($linkedElementsData, 'COLOR_XML_ID'));
$allColorXmlIds = array_filter($allColorXmlIds); // удаляем пустые

// Получаем данные из HL-блока (картинки цветов)
$hlData = [];
if (!empty($allColorXmlIds)) {
    $hlBlock = HL\HighloadBlockTable::getById(HL_BLOCK_ID)->fetch();
    if ($hlBlock) {
        $entity = HL\HighloadBlockTable::compileEntity($hlBlock);
        $entityDataClass = $entity->getDataClass();
        $rsColor = $entityDataClass::getList([
            'select' => ['UF_XML_ID', 'UF_NAME', 'UF_FILE'],
            'filter' => ['UF_XML_ID' => $allColorXmlIds]
        ]);
        while ($color = $rsColor->Fetch()) {
            $filePath = '';
            if (!empty($color['UF_FILE'])) {
                $filePath = CFile::GetPath($color['UF_FILE']);
            }
            $hlData[$color['UF_XML_ID']] = [
                'NAME'      => $color['UF_NAME'] ?: $color['UF_XML_ID'],
                'FILE_PATH' => $filePath,
                'FILE_ID'   => $color['UF_FILE']
            ];
        }
    }
}

// Наполняем каждый элемент массива информацией о цветах и ссылках связанных элементов
foreach ($arResult['ITEMS'] as &$item) {
    $itemId = $item['ID'];
    $linkedIds = $linkedByItem[$itemId] ?? [];
    $colors = [];
    $links = [];

    foreach ($linkedIds as $linkedId) {
        if (isset($linkedElementsData[$linkedId])) {
            $linked = $linkedElementsData[$linkedId];
            $links[$linkedId] = $linked['URL'];
            
            $xmlId = $linked['COLOR_XML_ID'];
            $colorData = null;
            
            // Проверяем наличие картинки из справочника
            if (!empty($xmlId) && isset($hlData[$xmlId]) && !empty($hlData[$xmlId]['FILE_PATH'])) {
                $colorData = $hlData[$xmlId];
                $colorData['IS_PREVIEW'] = false;
            } 
            // Если картинки цвета нет, используем картинку анонса связанного элемента
            elseif (!empty($linked['PREVIEW_PICTURE_PATH'])) {
                $colorData = [
                    'NAME' => $linked['NAME'],
                    'FILE_PATH' => $linked['PREVIEW_PICTURE_PATH'],
                    'FILE_ID' => null,
                    'IS_PREVIEW' => true
                ];
            }
            ...
            if ($colorData) {
                $colors[$linkedId] = $colorData;
            }
        }
    }

    $item['ASSOCIATED_COLORS_BY_ID'] = $colors;
    $item['ASSOCIATED_LINKS_BY_ID']   = $links;
}
unset($item);
