		...
		if(typeof(obPrices) == 'object')
		{
			...
			for(var j in obPrices)
			{
				if(obPrices[j])
				{
					...
					strPrice += '<div class="offers_price'+((obPrices[j].DISCOUNT_VALUE !== obPrices[j].VALUE) ? ' yellow-price' : '')+(arStikePrices ? (BX.util.in_array(obPrices[j].PRICE_ID, arStikePrices) ? ' strike_block' : '') : '')+'">';
