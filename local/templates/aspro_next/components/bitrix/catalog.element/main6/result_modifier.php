...
$HL_BLOCK_ID = 9; // Ваш ID

// ОСНОВНОЙ ЦВЕТ ТЕКУЩЕГО ЭЛЕМЕНТА (одиночное значение)
if (!empty($arResult["PROPERTIES"]["COLOR_MAIN_EL"]["VALUE"])) {
    $xmlId = $arResult["PROPERTIES"]["COLOR_MAIN_EL"]["VALUE"];
    ...
    if ($hlBlock) {
        ...
        $result = $entityDataClass::getList([
            'select' => ['UF_XML_ID', 'UF_NAME', 'UF_FILE'],
            'filter' => ['UF_XML_ID' => $xmlId],
            'limit' => 1
        ]);
        
        if ($hlItem = $result->Fetch()) {
            $arResult["MAIN_COLOR"] = [
                'NAME' => $hlItem['UF_NAME'] ?? $hlItem['UF_XML_ID'],
                'XML_ID' => $hlItem['UF_XML_ID'],
                'FILE_PATH' => CFile::GetPath($hlItem['UF_FILE']),
                'FILE_ID' => $hlItem['UF_FILE']
            ];
        }
    }
}

// ПРИВЯЗАННЫЕ ЭЛЕМЕНТЫ (у них COLOR_MAIN_EL тоже одиночное)
if (!empty($arResult["PROPERTIES"]["ASSOCIATED"]["VALUE"])) {
    
    $associatedIds = (array)$arResult["PROPERTIES"]["ASSOCIATED"]["VALUE"];
    
    // Получаем данные привязанных элементов
    $arFilter = [
    ...
    // Добавляем PREVIEW_PICTURE в выборку
    $arSelect = ["ID", "NAME", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "PROPERTY_COLOR_MAIN_EL"];
    ...
    while ($ob = $res->GetNextElement()) {
        ...
        // Получаем путь к картинке анонса, если она есть
        $previewPicturePath = '';
        ...
        $associatedElements[] = [
            ...
            "PREVIEW_PICTURE_PATH" => $previewPicturePath,
            "COLOR_XML_ID" => $arProps["COLOR_MAIN_EL"]["VALUE"] ?? '' // Одно значение
        ];
    }
    
    // Собираем все XML_ID цветов привязанных элементов
    $allXmlIds = [];
    foreach ($associatedElements as $element) {
        if (!empty($element["COLOR_XML_ID"])) {
            $allXmlIds[] = $element["COLOR_XML_ID"];
        }
    }
    ...
    if (!empty($allXmlIds)) {
        ...
        if ($hlBlock) {
            ...
            $result = $entityDataClass::getList([
                'select' => ['UF_XML_ID', 'UF_NAME', 'UF_FILE'],
                'filter' => ['UF_XML_ID' => array_unique($allXmlIds)]
            ]);
            
            while ($hlItem = $result->Fetch()) {
                $hlData[$hlItem['UF_XML_ID']] = [
                    'NAME' => $hlItem['UF_NAME'] ?? $hlItem['UF_XML_ID'],
                    'FILE_PATH' => CFile::GetPath($hlItem['UF_FILE']),
                    'FILE_ID' => $hlItem['UF_FILE']
                ];
    ...
    $arResult["ASSOCIATED_WITH_COLORS"] = [];
    foreach ($associatedElements as $element) {
        $colorData = null;
        
        // Проверяем наличие картинки из справочника
        $hasColorImage = false;
        if (!empty($element["COLOR_XML_ID"]) && isset($hlData[$element["COLOR_XML_ID"]])) {
            $colorData = $hlData[$element["COLOR_XML_ID"]];
            // Проверяем, существует ли файл картинки цвета
            if (!empty($colorData['FILE_PATH']) && file_exists($_SERVER['DOCUMENT_ROOT'] . $colorData['FILE_PATH'])) {
                $hasColorImage = true;
            }
        }
        
        // Если картинки цвета нет, используем картинку анонса
        if (!$hasColorImage && !empty($element["PREVIEW_PICTURE_PATH"])) {
            $colorData = [
                'NAME' => $element["NAME"],
                'FILE_PATH' => $element["PREVIEW_PICTURE_PATH"],
                'FILE_ID' => $element["PREVIEW_PICTURE"],
                'IS_PREVIEW' => true // Флаг, что это картинка анонса
            ];
        }
	...
	foreach($productSlider as $i => $arImage){
		$productSlider[$i] = array_merge(
			$arImage, array(
				...
				"THUMB" => CFile::ResizeImageGet($arImage["ID"], array("width" => 90, "height" => 90), BX_RESIZE_IMAGE_EXACT, true, array()),
			)
		);
	}
}
		...
		if($arOffer['MORE_PHOTO']){
			foreach($arOffer['MORE_PHOTO'] as $i => $arImage){
				if($arImage["ID"]){
					...
					$arOffer['MORE_PHOTO'][$i]["THUMB"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 92, "height" => 92), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
				}
			}
		}
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			$arOneRow = array(
				...
				'WEIGHT' => $arOffer['~CATALOG_WEIGHT'],
				'WIDTH' => $arOffer['~CATALOG_WIDTH'],
				'HEIGHT' => $arOffer['~CATALOG_HEIGHT'],
				'LENGTH' => $arOffer['~CATALOG_LENGTH'],
			...
			if($arOffer['PROPERTIES']['PRODUCT_VIDEO']['VALUE'] || $arResult['PROPERTIES']['POPUP_VIDEO']['VALUE']){
                $hasVideo = true;
                $arOneRow['PROD_VIDEO'] = ($arOffer['PROPERTIES']['PRODUCT_VIDEO']['VALUE'])?$arOffer['PROPERTIES']['PRODUCT_VIDEO']['VALUE']:$arResult['PROPERTIES']['POPUP_VIDEO']['VALUE'];
            }
...
if( !empty($arResult["PROPERTIES"]["ASSOCIATED"]["VALUE"]) ){
	...
	$arResult["ASSOCIATED"]=CNextCache::CIBLockElement_GetList(array('CACHE' => array("TAG" => CNextCache::GetIBlockCacheTag($arResult["PROPERTIES"]["ASSOCIATED"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["ASSOCIATED"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $arResult["PROPERTIES"]["ASSOCIATED"]["VALUE"]), false, false, $arSelect);
}
/*expandables*/
...
/*LINK_PORTFOLIO*/
if( !empty($arResult["PROPERTIES"]["LINK_PORTFOLIO"]["VALUE"]) ){
	$arSelect = array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "DETAIL_PAGE_URL");
	$arResult["LINK_PORTFOLIO"]=CNextCache::CIBLockElement_GetList(array('CACHE' => array("TAG" => CNextCache::GetIBlockCacheTag($arResult["PROPERTIES"]["LINK_PORTFOLIO"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["LINK_PORTFOLIO"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $arResult["PROPERTIES"]["LINK_PORTFOLIO"]["VALUE"]), false, false, $arSelect);
}
...
if($arResult["SECTION"])
{
	$arSection = CNextCache::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arResult["SECTION"]["ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "LEFT_MARGIN", "RIGHT_MARGIN", "NAME", "UF_TABLE_SIZES", "UF_COMMENT_PRICE"));
	...
	$arResult['UF_COMMENT_PRICE'] = $arSection['UF_COMMENT_PRICE'] ?? null;
}
...
$arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", "QUANTITY");
...
foreach ($arResult['JS_OFFERS'] as $k => $item):
$arResult['JS_OFFERS'][$k]['QUANTITY'] = $ar_predl[$item['ID']]['QUANTITY'];
endforeach;
...
/*canonical*/
$arResult['HAS_VIDEO'] = false;
if($hasVideo){
    $arResult['HAS_VIDEO'] = true;
}
