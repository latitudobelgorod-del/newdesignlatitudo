...
window.JCCatalogElement.prototype.initProductData = function()
{
	...
	if (!!this.params.PRODUCT && 'object' === typeof(this.params.PRODUCT))
	{
		...
		this.product.prtext = this.params.PRODUCT.PREVIEW_TEXT;
		...
		if($slider.length){
			...
			if(this.offers[index].ADDITIONAL_GALLERY.length){
				for(var i in this.offers[index].ADDITIONAL_GALLERY){
					...
					if(bTypeBig){
						slides += '<li class="col-md-12 item"><a href="' + arPhoto.DETAIL.SRC + '"  data-fancybox="gallery" data-caption="" class="sp-gallery-item-img-wrapper fancy fancybox" rel="media-gallery" target="_blank" title="' + arPhoto.TITLE + '"><img src="' + arPhoto.PREVIEW.src + '" class="img-responsive inline" title="' + arPhoto.TITLE + '" alt="' + arPhoto.ALT + '" /><span class="zoom"></span></li>';
...
window.JCCatalogElement.prototype.ChangeInfo = function()
{
	...
	if (-1 < index)
	{
		...
		setViewedProduct(this.offers[index].ID, {
			...
			'PREVIEW_TEXT': this.offers[index].PREVIEW_TEXT,
			'DETAIL_PAGE_URL': this.offers[index].URL,
			//'PICTURE_ID': this.offers[index].PREVIEW_PICTURE ? this.offers[index].PREVIEW_PICTURE.ID : (this.offers[index].SLIDER_COUNT ? this.offers[index].SLIDER[0].ID : false),
			'PICTURE_ID': this.offers[index].PREVIEW_PICTURE ? this.offers[index].PREVIEW_PICTURE.ID : (this.offers[index].PARENT_PICTURE ? this.offers[index].PARENT_PICTURE.ID : (this.offers[index].SLIDER_COUNT ? this.offers[index].SLIDER[0].ID : false)),
			'CATALOG_MEASURE_NAME': this.offers[index].MEASURE,
			'CATALOG_WEIGHT': this.offers[index].WEIGHT,
			'CATALOG_WIDTH': this.offers[index].WIDTH,
			'CATALOG_LENGTH': this.offers[index].LENGTH,
			'CATALOG_HEIGHT': this.offers[index].HEIGHT,
		...
		this.offerNum = index;
					if ((this.offers[index].QUANTITY) == '0') {
$(this.obProduct).find('.colproduct_block1').addClass('notavailability');
$(this.obProduct).find('.colproduct_block1').removeClass('availability');
$(this.obProduct).find('.colproduct_block1').text("Под заказ");

		}
	else {
	$(this.obProduct).find('.colproduct_block1').removeClass('notavailability');
	$(this.obProduct).find('.colproduct_block1').addClass('availability');
	$(this.obProduct).find('.colproduct_block1').text("В наличии");}
	...
	$('.product-article').html(this.offers[this.offerNum]['ID']); 
	$('.product-descr').html(this.offers[index].PREVIEW_TEXT);


	
	
if ((this.offers[this.offerNum]['HEIGHT']) || (this.offers[this.offerNum]['LENGTH']) || (this.offers[this.offerNum]['WIDTH']) || (this.offers[this.offerNum]['WEIGHT'] !=="0"))
{
	...
	{
		$('.logistic_title').css('display', 'none');
		...
		$('.logistic').css('display', 'block');
			...
			$('.product-tolwina').html('<span>Толщина:</span> ' + this.offers[this.offerNum]['HEIGHT'] + ' мм');
			...
			$('.product-dlina').css('display', 'none');
	}
	
	
	

	/*$('.product-weight').html(this.offers[index].WEIGHT);
	
	$('.product-tolwina').html(this.offers[index].HEIGHT);
	$('.product-shirina').html(this.offers[index].WIDTH);
	$('.product-dlina').html(this.offers[index].LENGTH);*/
};
...
window.JCCatalogElement.prototype.SetSliderPict = function(obj, slider, config)
{
	var container=$('.wrapp_thumbs'),
		...
		container.find('.thumbs').css({
			'max-width':Math.ceil(((countPhoto <= 5 ? countPhoto : 5) * 104) - 10)
		});
		...
		for(var i in slider){
			if(typeof(slider[i]) == 'object'){
				...
				'<span class="cnt"><img border="0" src="'+slider[i].THUMB.src+'" alt="'+slider[i].ALT+'" title="'+slider[i].TITLE+'" /></span>'+'</li>';
			}
		}
		slideHtml+='</ul>';
	...
	if(!slider.length){
		...
		for(var i in slider){
			if(typeof(slider[i]) == 'object'){
				...
				'<a href="'+slider[i].BIG.src+'" data-fancybox-group="item_slider" class="fancy popup_link" title="'+slider[i].TITLE+'"><img border="0" src="'+slider[i].SMALL.src+'" alt="'+slider[i].ALT+'" title="'+slider[i].TITLE+'" /></a>'+
				'</li>';
			}
		}
	}
...
window.JCCatalogElement.prototype.setBuyBlock = function(th, obj)
{
	...
	var className=((obj.CONFIG.ACTION == "ORDER") || !obj.CAN_BUY || !obj.CONFIG.OPTIONS.USE_PRODUCT_QUANTITY_DETAIL || (obj.CONFIG.ACTION == "SUBSCRIBE" && obj.CATALOG_SUBSCRIBE == "Y") ? "wide" : "" ),
		buyBlockBtn=$('<div class="button_block" style="width: 60%;  float: right;  box-sizing: border-box;"></div>');
			...
			for(var j in obPrices)
			{
				if(obPrices[j])
				{
					...
					strPrice += '<div class="offers_price'+(arStikePrices ? (BX.util.in_array(obPrices[j].PRICE_ID, arStikePrices) ? ' strike_block' : '') : '')+'">';
						strPrice += '<span class="values_wrapper">'+'Цена: '+getCurrentPrice(obPrices[j].DISCOUNT_VALUE, obPrices[j].CURRENCY, obPrices[j].PRINT_DISCOUNT_VALUE)+'</span>';
						if(measure)
							strPrice += '<span class="price_measure">/'+measure+'</span>';
