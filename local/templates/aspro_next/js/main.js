...
$(document).ready(function() {
  ...
  $('.accordeon_lat_footer .acc-head-main').on('click', f_acc_footer);
});

function f_acc_footer(){
//скрываем все кроме того, что должны открыть
  $('.accordeon_lat_footer .acc-body').not($(this).next()).slideUp();
...
document.addEventListener("DOMContentLoaded", function() {
 ...
 $(".opentime").text("Офис открыт + все услуги доступны удаленно. Звоните!");
$("#footer .opentime").text("Магазин шоурум открыт + все услуги доступны удаленно. Звоните!");
$(".closetime").text("Сейчас офис закрыт, закажите звонок");
$("#footer .closetime").text("Магазин шоурум сейчас закрыт, закажите звонок");
$(".sticker_vybor_pokupateley").text("Выбор покупателей");
$(".sticker_sovetuem").text("Советуем");
$(".sticker_novinka").text("Новинка");
$(".sticker_aktsiya_rasprodazha").text("Акция / Распродажа");
$(".sticker_aktsiya").text("Акция");



	$( document ).ready(function(){
  $( ".info_item .price_matrix_wrapper .price .price_value" ).before( "Цена: " ); 
	});
...
if (!funcDefined("onLoadjqm")) {
  var onLoadjqm = function (name, hash, requestData, selector, requestTitle, isButton, thButton, name2) {
	  var text = $(hash.t).text(), head = $('div.form_head h2.formnameru'),
						headinline = $('div.form_headin h2.formnameruinline'), urlform = window.location.href, urlforminline = document.location.href, domenurl = window.location.host,domenurlinline = location.host, form = $('div.form_head').parent().find('form');
				if (text){
                            form.attr('action', form.attr('action') + '&formhead=' + encodeURIComponent(text));
						}
	if (head.length)
						{
							head.text(text);								
							$('input[data-sid="NAMEFORM"]', form).val(text);
							//$('input[data-sid="URLFORM"]').val(urlform);
						//	$('input[data-sid="DOMENURL"]').val(domenurl);							
						}						
	  	$('input[data-sid="DOMENURL"]').val(domenurl);
				$('input[data-sid="DOMENURLINLINE"]').val(domenurlinline);				
			
				$('input[data-sid="URLFORM"]').val(urlform);
				$('input[data-sid="URLFORMINLINE"]').val(urlforminline);
...
$(document).ready(function () {
  ...
  MegaMenuFixed();
  	$("img.lazy").lazyload({
			...
			var head = $('.maxwidth-theme').prev('h2:first');
		if (head.length){
		var form = $('.maxwidth-theme').find('form'), text = head.text(), urlform1 = window.location.href, urlforminline = document.location.href,domenur1 = window.location.host;
		if (form.length){
			form.attr('action', form.attr('action') + '&formhead=' + encodeURIComponent(text));
			$('input[data-sid="NAMEFORM"]', form).val(text);
	...
	$('input[data-sid="DOMENURL"]').val(domenur1);	
	
	$('input[data-sid="URLFORM"]').val(urlform1);
			$('input[data-sid="URLFORMINLINE"]').val(urlforminline);
  ...
  try {
    if (!window.renderRecaptchaById || !window.asproRecaptcha || !window.asproRecaptcha.key) {
     eventdata.form.submit();
			...
			if(eventdata.form.name==="WHATSAPP")
			ym(62259859,'reachGoal','SUBMITRUFORM');
			
			if(eventdata.form.name==="MAINFORM")
			ym(62259859,'reachGoal','SUBMITRUFORM');
			
			if(eventdata.form.name==="TOORDER")
			ym(62259859,'reachGoal','SUBMITRUFORM');
