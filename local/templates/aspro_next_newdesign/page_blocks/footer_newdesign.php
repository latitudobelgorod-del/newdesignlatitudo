...
<div class="nd-footer">
	...
	<div class="nd-footer__bottom">
		<div class="nd-footer__inner">
			...
			<?/* Юридическая оговорка. Текст тот же, что в подвале старого дизайна
			   (include/footer/footer_block/footer_6_col.php). Домен подставляем
			   текущий: на региональных поддоменах ссылка должна вести на их адрес.
			   Берём HTTP_HOST, а не SERVER_NAME, как в старом подвале: SERVER_NAME
			   отдаёт то, что записано в конфиге виртуального хоста, и на поддомене
			   это может оказаться главный домен. */?>
			<div class="nd-fnote">
				Вся представленная на сайте информация (в том числе стоимость товаров,
				их характеристики, информация о наличии, стоимость, состав и порядок
				предоставления услуг) носит информационный характер и ни при каких
				условиях не является публичной офертой, определяемой положениями
				Статьи 437(2) Гражданского кодекса РФ. При использовании любых
				материалов сайта, включая фотографии и тексты, активная ссылка на
				<a href="/">https://<?=htmlspecialcharsbx((string)($_SERVER['HTTP_HOST'] ?: $_SERVER['SERVER_NAME']))?></a>
				обязательна.
			</div>
		</div>
	</div>
