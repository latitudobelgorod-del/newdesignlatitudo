...
(function () {
    ...
    /* ---------------------------------------------------------------------
       Плитка акции — последняя ячейка КАЖДОГО ВТОРОГО ряда: 2-й, 4-й, 6-й
       (Ирина, 2026-08-11, по макету Figma). Было «через каждые четыре ряда»,
       и в разделе с тремя акциями вторая и третья метили в 6-й и 10-й ряд —
    ...
    function placePromo() {
        try {
            ...
            all.forEach(function (promo, i) {
                /* Готовая сетка: ряд 2 → ячейка cols*2, ряд 4 → cols*4 и так далее.
                ...
                var itemsBefore = cols * (2 + i * 2) - 1 - i;
