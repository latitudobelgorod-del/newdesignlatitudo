...
<?// Заголовок блока — один на все проекты, как в макете: «Вас может
   // заинтересовать» (Ирина, 10 сентября 2026). В редакторе у каждого
   // проекта он набран руками, и вариантов набралось семь («Вас также
   // может…», «Также вас могут…», с опечатками). Печатаем свой, а заголовок
   // из редактора (блок htag — единственные h2–h4 в этой области, карточки
   // проектов заголовков не содержат) вырезаем из готовой разметки.?>
<?if($ndEditor2Below):?>
	<div class="nd-projinterest">
		<h2 class="nd-projinterest__title">Вас может заинтересовать</h2>
		<div class="editor">
		<?ob_start();?>
		<?$APPLICATION->IncludeComponent(
		...
		<?=preg_replace('~<h([2-4])\b[^>]*>.*?</h\1>~su', '', ob_get_clean())?>
		</div>
	</div>
<?endif;?>
