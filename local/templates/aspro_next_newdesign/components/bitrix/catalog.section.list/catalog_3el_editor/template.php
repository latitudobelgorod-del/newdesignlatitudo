...
<? $imgSect = (empty($arSection["PICTURE"]["SRC"]) ? 'noimng' : '');?>
				...
				<div class="item <?=(empty($arSection["PICTURE"]["SRC"]) ? 'noimng' : '')?>" id="<?=$this->GetEditAreaId($arSection['ID']);?>">
					...
					<?endif;?>
<div class="<?=(empty($arSection["PICTURE"]["SRC"]) ? '' : 'name')?> " >
					<div class="blk"> 
					<div class="left col-md-12 col-sm-12"><a href="<?=$arSection['SECTION_PAGE_URL'];?>" class="<?=(empty($arSection["PICTURE"]["SRC"]) ? 'dark_link' : 'white_link')?> " ><?=$arSection['NAME'];?></a></div>
