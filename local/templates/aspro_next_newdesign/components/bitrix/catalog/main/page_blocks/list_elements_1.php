...
?>
<?
/* Плитки подразделов показываем и при выбранном фильтре (адрес с /filter/):
   раньше весь ряд прятался, и, отметив пункт фильтра, посетитель терял
   навигацию по соседним разделам (Ирина, 18 сентября 2026). Баннер акции и
   КМ «между подразделами и элементами» на страницах фильтра по-прежнему не
   выводятся — они собираются только для чистого адреса раздела. */
$ndIsFilterPage = (bool) substr_count($_SERVER['REQUEST_URI'], '/filter/');
$ndAkciyaHtml = $ndAkciyaHtml ?? '';
?>
	<?
	...
	if ($ndMenuLinkCards !== '') {
		if ($ndSubsecHtml !== '') {
			/* Кладём карточки внутрь сетки компонента, перед её закрывающим тегом,
			   чтобы они попали в тот же грид, а не встали отдельным рядом. */
			$ndGridEnd = strrpos($ndSubsecHtml, '</div>');
			$ndSubsecHtml = ($ndGridEnd !== false)
				? substr($ndSubsecHtml, 0, $ndGridEnd) . $ndMenuLinkCards . substr($ndSubsecHtml, $ndGridEnd)
				: $ndSubsecHtml . $ndMenuLinkCards;
		} else {
			$ndSubsecHtml = '<div class="nd-subsec">' . $ndMenuLinkCards . '</div>';
		}
	}
	?>
	...
	<? endif; ?>
	<? if ($iSectionsCount && !$ndIsFilterPage): ?>
...
$ndSiblingRowSections = array(558, 559);   // «Ограждения Polivan», «Ограждения Террапол»
$ndSibGroupId = 0;
if (!empty($ndRowHtml)) {
...
<? if (!$arSeoItem): ?>
            ...
            <? endif; ?>
<? endif; ?>
<?/* Теги посадочных и плашка о цене — и на посадочных (21.09.2026, Ирина: «теги должны быть
     даже на странице фильтра»). Раньше весь блок стоял под if (!$arSeoItem), и на посадочной
     не было ни тегов соседних посадочных, ни UF_COMMENT_PRICE. Под условием выше остался
     только noindex технических страниц фильтра. Для посадочной теги — соседние посадочные
     того же раздела каталога без неё самой (ветка $arSeoItem ниже). */?>
<? if (true): ?>
...
<?if($itemsCnt):?>
			...
			<?/* Плитки акций занимают ячейки той же сетки, поэтому уменьшаем на
			   столько же число товаров на странице — иначе последний ряд
			   остаётся неполным. Саму разметку печатаем ниже, после списка.

			   Вычитаем не все акции раздела, а только те, которым на странице
			   хватает места (Ирина, 2026-08-11): плитка встаёт в каждый второй
			   ряд, значит их влезает floor(рядов / 2). В разделе террасной
			   доски акций три, а рядов пять — вставали две, вычиталось три, и
			   в конце оставалась пустая ячейка. Остальные ждут в пуле и
			   встают после «Показать ещё» (js/newdesign-catalog.js).

			   Колонок в сетке 4, на мобильном 2 — как и число товаров, берём по
			   isMobilelat(). На планшете колонок 3 и ровного ряда всё равно не
			   выходит: 20 на 3 не делится. */?>
			<? include(__DIR__ . "/../include/promo_catalog_newdesign.php") ?>
			<?
			$ndPromoCount = (int)$GLOBALS['ND_CATALOG_PROMO_COUNT'];
			if($ndPromoCount > 0){
				$ndCols = (isMobilelat() ? 2 : 4);
				$ndPromoFit = (int)floor((int)floor($show / $ndCols) / 2);
				$ndPromoShown = min($ndPromoCount, $ndPromoFit);
				if($ndPromoShown > 0 && $show > $ndPromoShown){
					$show = $show - $ndPromoShown;
				}
			}
...
<? if (!IsSeoDisrupting($arParams)): ?>				
 ...
 <? else: ?>
					...
					<?if ($arSeoItem["PROPERTY_NOINDEX_VALUE"]):?>
						...
						<?else:?> 
					<?endif?>
					<?/* Список посадочных Аспро на страницах пагинации (?PAGEN_1=2 и дальше) больше
					   не выводим (Ирина, 22.09.2026): он печатался внизу в оформлении старого
					   дизайна и дублировал чипы посадочных, которые уже стоят в строке
					   сортировки (их собирает блок «Популярные категории» выше). Ветка
					   осталась ради noindex посадочной с PROPERTY_NOINDEX. */?>
                    <? endif; ?>
                    <? /*Популярные категории*/ ?>
