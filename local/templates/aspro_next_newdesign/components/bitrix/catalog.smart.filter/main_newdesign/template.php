...
// стоит у всех свойств, поэтому решаем здесь: открываем две первых плюс те,
// в которых уже что-то выбрано, — иначе активный фильтр спрячется.
$ndBoxIndex = 0;
$ndIsExpanded = function($arItem) use (&$ndBoxIndex) {
	$ndBoxIndex++;
	if ($ndBoxIndex <= 2) {
		return true;
	}
	if (is_array($arItem["VALUES"])) {
		foreach ($arItem["VALUES"] as $ndValue) {
			if (!empty($ndValue["CHECKED"])) {
				return true;
			}
		}
	}
	...
	<div class="bx_filter bx_filter_vertical swipeignore nd-filter">
		<div class="bx_filter_section">
			<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
				<div class="nd-filter__title"><?=Loc::getMessage("FILTER_TITLE");?></div>
				...
				<div class="bx_filter_parameters">
					...
					foreach($arResult["ITEMS"] as $key=>$arItem)
					{
						...
						if(isset($arItem["PRICE"])):
							...
							?>
							<?$ndBoxIndex++;?>
							<div class="bx_filter_parameters_box active price_numbers">
								<span data-f="<?=Loc::getMessage('CT_BCSF_SET_FILTER')?>" data-fi="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TI')?>" data-fr="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TR')?>" data-frm="<?=Loc::getMessage('CT_BCSF_SET_FILTER_TRM')?>" class="bx_filter_container_modef"></span>
												...
												<div class="bx_filter_parameters_box_container_block">
													<div class="bx_filter_input_container form-control bg">
														<input
															...
															size="5"
															placeholder="<?=Loc::getMessage("CT_BCSF_FILTER_PRICE_FROM")?> <?echo $price1;?>"
												...
												<div class="bx_filter_parameters_box_container_block">
													<div class="bx_filter_input_container form-control bg">
														<input
															...
															size="5"
															placeholder="<?=Loc::getMessage("CT_BCSF_FILTER_PRICE_TO")?> <?echo $price5;?>"
															onkeyup="smartFilter.keyup(this)"
														/>
													</div>
												</div>
					...
					foreach($arResult["ITEMS"] as $key=>$arItem)
					{
						...
						if (
							...
							continue;
						$class = $ndIsExpanded($arItem) ? "active" : "";
						$isFilter=true;
