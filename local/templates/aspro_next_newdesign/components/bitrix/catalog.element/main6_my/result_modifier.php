		...
		if($arOffer['MORE_PHOTO']){
			foreach($arOffer['MORE_PHOTO'] as $i => $arImage){
				if($arImage["ID"]){
					...
					$arOffer['MORE_PHOTO'][$i]["THUMB"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 92, "height" => 92), BX_RESIZE_IMAGE_EXACT, true, array());
				}
			}
		}
...
/*
if(($arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"])||($arResult["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"]))
{
if($arResult["IPROPERTY_VALUES"]["ELEMENT_PAGE_TITLE"])
	...
	if($arResult["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"])
		$goy=$arResult["IPROPERTY_VALUES"]["ELEMENT_META_TITLE"];
		else 
			$goy=$arResult["NAME"];
}
