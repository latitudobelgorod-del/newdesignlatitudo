...
<span itemscope="" itemtype="http://schema.org/Product" style="display: none">
    ...
    <meta itemprop="description" content="<?=!empty($arResult['DESCRIPTION'])?$arResult['DESCRIPTION']:$arResult['IPROPERTY_VALUES']['SECTION_META_DESCRIPTION']?>"/>
    <span itemtype="http://schema.org/AggregateOffer" itemscope="" itemprop="offers">
        <meta content="<?=count($arResult["ITEMS"])?>" itemprop="offerCount">
        <meta content="<?=$arResult['PRICES_RANGE']['MAX']?>" itemprop="highPrice">
        <meta content="<?=$arResult['PRICES_RANGE']['MIN']?>" itemprop="lowPrice">
...
$regionID = ($arRegion ? $arRegion['ID'] : '');?>
	...
	<?if($arParams["AJAX_REQUEST"]=="N" ){?>
		<div class="top_wrapper row margin0 <?=($arParams["SHOW_UNABLE_SKU_PROPS"] != "N" ? "show_un_props" : "unshow_un_props");?>">
			<div id="portfolio_loader" class="catalog_block  items block_list news-list ">
...
<?//        echo '<pre>', print_r($arResult['SKU_PROPS']), '</pre>' ; ?>
        <script src="/bitrix/components/maxyss/measure_unit/templates/aspro_list_tp/script.js"></script>
        <link rel="stylesheet" href="/bitrix/components/maxyss/measure_unit/templates/aspro_list_tp/style.css">
...
?>
				<div class="catalog_item_wrapp item  news-item">
...
$renderImg = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE'], Array("width" => 100, "height" => 100), BX_RESIZE_IMAGE_PROPORTIONAL, false);
	...
	<div class="article_block item-article" style="color: #d0d0d0;font-size: 8px;position: absolute;top: 0;right: 5px;">
			...
			<?else:?>
			<?if ($isArticle): ?>
							<div style="color: #d0d0d0;font-size: 8px;position: absolute;top: 0;right: 5px;">Артикул: <?=$isArticle?>	</div>
			...
			<?endif;?>
								...
								<? if ($arItem["PROPERTIES"]["PROP_2065"]["VALUE"]): ?>
								<?=$arItem["PROPERTIES"]["PROP_2065"]["NAME"];?> - <?=$arItem["PROPERTIES"]["PROP_2065"]["VALUE"];?><br>


								<? endif; ?>

								<? if ($arItem["PROPERTIES"]["IT_8"]["VALUE"]): ?>
								<?=$arItem["PROPERTIES"]["IT_8"]["NAME"];?> - <?=$arItem["PROPERTIES"]["IT_8"]["VALUE"];?><br>
								<? endif; ?>

								<? if ($arItem["PROPERTIES"]["IT_6"]["VALUE"]): ?>
								<?=$arItem["PROPERTIES"]["IT_6"]["NAME"];?> - <?=$arItem["PROPERTIES"]["IT_6"]["VALUE"];?>
								<? endif; ?>
...
<script>
$(document).ready(function() {
$( ".catalog_block .cost.prices .price" ).prepend( "от " );
							});
</script>
<?endif;?>
