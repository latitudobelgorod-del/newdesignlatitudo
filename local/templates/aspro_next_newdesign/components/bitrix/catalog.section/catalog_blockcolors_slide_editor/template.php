...
<?global $arRegion;
  $regionID = ($arRegion ? $arRegion['ID'] : '');?>
	...
	<?if(($arParams["AJAX_REQUEST"]=="N") || !isset($arParams["AJAX_REQUEST"])){?>
	 ...
	 <div id="portfolio_loader" class="catalog_block items margin0  flexbox"> 
	<?}?>
...
?>
	...
	<?foreach($arResult["ITEMS"] as $i => $arItem){?>
		<li class="item_block visible col-lg-3 col-md-3  col-sm-3 col-xs-3">
...
<?/* Смотрите также / Вариации */?>
			...
			<?endforeach;?>
				...
				<link itemprop="url" href="<?=$arItem["DETAIL_PAGE_URL"]?>" />
                                    ...
                                    <?}else{?>
                                        ...
                                        {?>
                                            ...
                                            <?if (is_array($arItem['PROPERTIES']['UNIT_KOEF']['VALUE'])) {
                                                ...
                                                ?>
                                                    <span style="display: none"><?= $arItem['MIN_PRICE']['DISCOUNT_VALUE'] ?> руб./<?echo $arItem['ITEM_MEASURE']['TITLE'] ?></span>
                                                <?
                                            }
                                        }else{
                                            ...
                                            $min_price_id=0;?>
                                                ...
                                                <?if (is_array($arItem['PROPERTIES']['UNIT_KOEF']['VALUE'])) {
                                                    ...
                                                    ?>
                                                    <span style="display: none"><?= $arItem['PRICES']['BASE']['DISCOUNT_VALUE'] ?> руб./<?echo $arItem['ITEM_MEASURE']['TITLE'] ?></span>
                                                    <?
                                                }
                                            ?>
                                        <?}?>
                            ...
                            <?elseif($arItem["OFFERS"]):?>
                                ...
                                <?}else{?>
                                    <?
                                    $buttonHTML = '<span class="small read_more1 to-order btn btn-default grey transition_bg transparent animate-load" data-event="jqm" data-param-form_id="TOORDER" data-name="toorder" data-autoload-product_name="" data-autoload-product_id="'.$arItem["ID"].'"><i></i>
        ...
        <?if(isset($arItem['OFFERS']) && count($arItem['OFFERS'])>0){
            ...
            foreach ($arItem['OFFERS'] as $key=>$offer_u){
                ...
                if(!empty( $offer_u["PROPERTIES"]["UNIT_KOEF"]["VALUE"]))$offer_unit_yes = true;
