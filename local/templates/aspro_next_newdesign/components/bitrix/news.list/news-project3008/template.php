...
<?endif;?>
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<div class="col-md-<?=floor(12 / $arParams['LINE_ELEMENT_COUNT_LIST'])?> col-sm-<?=floor(12 / round($arParams['LINE_ELEMENT_COUNT_LIST'] / 2))?> <?=$class;?>" data-ref="mixitup-target">
					<div class="item shadow slice-item noborder<?=($bImage ? '' : ' wti')?><?=($bActiveDate ? ' wdate' : '')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
						<?if($imageSrc):?>
							<div class="image">
								...
								<?endif;?>
									<img class="lazy" data-src="<?=$res_images["src"]?>"  alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-responsive" >
									<?if($bDetailLink):?>
									</a>
								<?endif;?>
					...
					<?if(((isset($arItem['PROPERTIES']['REVIEW']) && $arItem['PROPERTIES']['REVIEW']['~VALUE']['TEXT'])) || ($arItem['PROPERTIES']['LINK_YOUTUBE']['VALUE'])):?>
					<div class="stickers">
					
					<?if($arItem['PROPERTIES']['LINK_YOUTUBE']['VALUE']):?>
					<div class="sticker_video">Видеообзор</div>
					<?endif;?>
