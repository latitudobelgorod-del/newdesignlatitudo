...
<div class="col-md-12">
					<div class="item noborder<?=($bImage ? '' : ' wti')?><?=($bActiveDate ? ' wdate' : '')?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
						<?if($bImage):?>
							...
							<img src="<?=SITE_TEMPLATE_PATH?>/images/no_photo_medium.png" alt="<?=$arItem['NAME'];?>" title="<?=$arItem['NAME'];?>" class="img-responsive"  style="margin:0 auto;" />
							<?if($bDetailLink):?></a>
							<?endif;?>
</div>
						<?endif;?>
...
?>
			...
			<?else:?><div class="akciya_end" >Акция завершена</div>
			<?endif;?>							<?endif;?>
<div class="title"><?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="dark-color"><?endif;?><?=$arItem['NAME']?><?if($bDetailLink):?></a><?endif;?></div>
<?endif;?>
<?
