...
<?endif;?>
			...
			<?foreach($arResult['ITEMS'] as $i => $arItem):?>
				...
				<div class="col-md-<?=floor(12 / $arParams['LINE_ELEMENT_COUNT_LIST'])?> col-sm-<?=floor(12 / round($arParams['LINE_ELEMENT_COUNT_LIST'] / 2))?> col-xs-12 <?=$class;?>" data-ref="mixitup-target">
					<div class="item shadow slice-item noborder<?=($bImage ? '' : ' wti')?><?=($bActiveDate ? ' wdate' : '')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
						<?if($imageSrc):?>
							<div class="image">
								...
								<?endif;?>
									<img  src="/assets/lazyload/loading.gif" data-original="<?=$res_images["src"]?>"  alt="<?=($bImage ? $arItem['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="lazy img-responsive" >
									<?if($bDetailLink):?>
									</a>
								<?endif;?>
					...
					<?if(((isset($arItem['PROPERTIES']['REVIEW']) && $arItem['PROPERTIES']['REVIEW']['~VALUE']['TEXT'])) || ($arItem['PROPERTIES']['VIDEO']['VALUE']) || ($arItem['PROPERTIES']['GALLEY_BIG']['VALUE'])):?>
					<div class="stickers">
					...
					$count_photo++;?>
					<?endforeach;?>
					<div class="sticker_photo" ><div class="fa-caret"><i class="fa fa-camera"></i></div><?=$count_photo?> фото</div>
					<?endif;?>
