	...
	BX.Sale.OrderAjaxComponent = {
		...
		initOptions: function()
		{
			...
			if (this.result.TOTAL)
			{
				...
				this.options.showOrderLenght = total.ORDER_LENGTH && parseFloat(total.ORDER_LENGTH) > 0;
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			else if (BX.util.in_array(column.id, ["QUANTITY", "WEIGHT_FORMATED", "LENGTH_FORMATED", "DISCOUNT_PRICE_PERCENT_FORMATED"]))
			{
...
if (this.options.showOrderLength)
			{
				this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message('SOA_SUM_LENGTH_SUM'), total.ORDER_LENGTH_FORMATED));
			}
