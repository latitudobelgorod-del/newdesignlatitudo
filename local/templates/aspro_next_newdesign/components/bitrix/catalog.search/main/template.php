...
if (is_array($arElements) && !empty($arElements)) {
	...
	if ($arParams["USE_FILTER"] === "Y") {
		...
		/* Возвращаем условие по найденным ID: компонент фильтра пишет
		   выбранные значения в ту же глобалку и наш список затирает. После
		   этого выдача переставала зависеть от запроса — по «террасная доска
		   бежевый» с фильтром длины показывались столбы со всего каталога
		   (Ирина, 7 сентября 2026). Ставим поверх выбранных условий, чтобы
		   работало пересечение: и запрос, и фильтр. */
		$searchFilter['=ID'] = $arElements;

		/* FACET_OPTIONS — служебные настройки цены для фасетного индекса. Умный
		   фильтр кладёт их в ту же глобалку, как только у каталога включён
		   индекс (16 сентября 2026). catalog.section сам их убирает, только
		   если других условий нет, а у нас есть список ID — ключ уходил в
		   выборку, и CIBlockElement::GetList возвращал ноль: «Найдено 0» на
		   любой запрос. Списку эти настройки не нужны — убираем, как делает
		   Аспро на странице бренда (news/partners_origin/element_4.php). */
		unset($searchFilter['FACET_OPTIONS']);

		/* Если фильтруют по свойству торгового предложения (длина, цвет), в
		   фильтре появляется подмассив OFFERS. Список товаров по нему строит
		   сам компонент: находит подходящие предложения и подменяет условие
		   по ID их родителями — вместе с нашим списком. Поэтому ограничиваем
		   и выборку предложений: берём только те, что принадлежат найденным
		   товарам (свойство «Элемент каталога», 167). Тогда что бы компонент
		   ни подставил, из нашего списка он не выйдет. */
		if(!empty($searchFilter['OFFERS']) && is_array($searchFilter['OFFERS']))
			$searchFilter['OFFERS']['PROPERTY_'.LatitudoQuickSearch::PROP_OFFER_LINK] = $arElements;
	}
