    ...
    if ($arResult['imageUploadFrame'] == 'Y') {
        ...
        if (!$bAjaxPost && $arResult['CanUserComment']) {
            ...
            <div id="form_comment_" style="display:none;">
                ...
                if (isset($request['IBLOCK_ID'])) {
                    ?><input type="hidden" name="IBLOCK_ID" value="<?=(int) $request['IBLOCK_ID'];?>"><?php
                }
            if (isset($request['ELEMENT_ID'])) {
                ?><input type="hidden" name="ELEMENT_ID" value="<?=(int) $request['ELEMENT_ID'];?>"><?php
            }
            if (isset($request['SITE_ID'])) {
                ?><input type="hidden" name="SITE_ID" value="<?=htmlspecialcharsbx($request['SITE_ID']);?>"><?php
            }
                    ...
                    if ($arResult['COMMENT_PROPERTIES']['SHOW'] == 'Y') {
                        ...
                        foreach($arResult['COMMENT_PROPERTIES']['DATA'] as $FIELD_NAME => $arPostField) {
                            if ($FIELD_NAME == 'UF_BLOG_COMMENT_DOC') {
                                ?><a id="blog-upload-file" href="javascript:blogShowFile()"><?=GetMessage('BLOG_ADD_FILES');?></a><?php
                            }
                            ?>
                            <div id="blog-comment-user-fields-<?=$FIELD_NAME;?>"><?=$FIELD_NAME == 'UF_BLOG_COMMENT_DOC' ? '' : $arPostField['EDIT_FORM_LABEL'].':';?>
                                <?$APPLICATION->IncludeComponent('bitrix:system.field.edit', $arPostField['USER_TYPE']['USER_TYPE_ID'], ['arUserField' => $arPostField], null, ['HIDE_ICONS' => 'Y']);?>
                            </div>
                            <?php
                        }
                ...
                if ($comment['PUBLISH_STATUS'] == BLOG_PUBLISH_STATUS_PUBLISH || $comment['SHOW_SCREENNED'] == 'Y' || $comment['ID'] == 'preview') {
                    ...
                    <div class="blog-comment-info">
                        ...
                        if ($tabCount > 0 || $comment['PARENT_ID']) {
                            echo CNext::showIconSvg('arrow_answer', SITE_TEMPLATE_PATH.'/images/svg/arrow_answer.svg');
                        }
                        ?>
                    ...
                    <?if (isset($comment['UF_ASPRO_COM_APPROVE']) && $comment['UF_ASPRO_COM_APPROVE']):?>
                        <div class="blog-comment-approve alert alert-success alert-success--with-icon font_sxs">
                            <?=CNext::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/content_icons_sprite.svg#icon-checkmark', 'svg-checkmark', ['HEIGHT' => 7, 'WIDTH' => 10]);?>
                            ...
                            foreach ($comment['COMMENT_PROPERTIES']['DATA'] as $FIELD_NAME => $arPostField) {
                                if (!empty($arPostField['VALUE'])) {
                                    $GLOBALS['APPLICATION']->IncludeComponent(
                                        ...
                                        ['arUserField' => $arPostField], null, ['HIDE_ICONS' => 'Y']);
                                }
                            }
                            ?></div><?php
                            if ($eventHandlerID !== false && (intval($eventHandlerID) > 0)) {
                    ...
                    <?php
                    if (
                        (strlen($errorComment) > 0 || strlen($_POST['preview']) > 0)
                        && (intval($_POST['parentId']) == $comment['ID'] || intval($_POST['edit_id']) == $comment['ID'])
                        ...
                        if (intval($_POST['edit_id']) == $comment['ID']) {
                            ?>editComment('<?=$comment['ID'];?>');<?php
                        } else {
                            ?>showComment('<?=$comment['ID'];?>', 'Y', '<?=CUtil::JSEscape($_POST['user_name']);?>', '<?=CUtil::JSEscape($_POST['user_email']);?>', 'Y');<?php
                        }
                        ?>
                        </script>
                        <?php
        ...
        if (!$bAjaxPost) {
            if ($arResult['CanUserComment']) {
                ...
                <?php
                if (
                    strlen($arResult['COMMENT_ERROR']) > 0 && strlen($_POST['parentId']) < 2
        ...
        if (!$bAjaxPost && $arResult['NEED_NAV'] == 'Y') {
            for ($i = 1; $i <= $arResult['PAGE_COUNT']; ++$i) {
                ...
                ?>
                    <div id="blog-comment-page-<?=$i;?>"<?if ($arResult['PAGE'] != $i) {
                        echo 'style="display:none;"';
                    }?>><?RecursiveComments($tmp, $arResult['firstLevel'], 0, true, $arResult['canModerate'], $arResult['User'], $arResult['use_captcha'], $arResult['CanUserComment'], $arResult['COMMENT_ERROR'], $arResult['Comments'], $arParams);?></div>
                <?php
            }
        } else {
            ...
            if ($arResult['NEED_NAV'] == 'Y') {
                ...
                <div class="blog-comment-nav">
                    ...
                    for ($i = 1; $i <= $arResult['PAGE_COUNT']; ++$i) {
                        ...
                        ?><a class="<?=$style;?>" href="<?=$arResult['NEW_PAGES'][$i];?>" onclick="return bcNav('<?=$i;?>', this)" id="blog-comment-nav-b<?=$i;?>"><?=$i;?></a><?php
                    }
                ?>
                </div>
                <?php
