...
foreach ($this->basketItems as &$row)
{
	...
	$rowData = array(
		...
		'HEIGHT' => $row['HEIGHT'] ?? ($dimensions['HEIGHT'] ?? ''),
		'WIDTH' => $row['WIDTH'] ?? ($dimensions['WIDTH'] ?? ''),
		'LENGTH' => $row['LENGTH'] ?? ($dimensions['LENGTH'] ?? ''),
		'VOLUME' => $row['VOLUME'] ?? ($dimensions['VOLUME'] ?? ''),
		'WEIGHT' => $row['WEIGHT'] ?? ($dimensions['WEIGHT'] ?? ''),
		'PROPS' => $row['PROPS'],
