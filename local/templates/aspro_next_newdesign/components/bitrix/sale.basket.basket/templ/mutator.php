 ...
 */
 $dbBasketItems = CSaleBasket::GetList(
        ...
        false,
        false,
        array("ID","PRODUCT_ID","QUANTITY", "LENGTH", "WEIGHT"));
...
{
//    $dimensions = $this->basketDimensions($row);
	$rowData = array(
		'ID' => $row['ID'],
	...
	if ($rowData['LENGTH']){
        $row['LENGTH_FORMATED'] = $rowData['LENGTH'] . ' мм';
    }

   

    if ($rowData['WIDTH']){
        $row['WIDTH_FORMATED'] = $rowData['WIDTH'] . ' мм';
    }
	
	
 if ($rowData['HEIGHT']){
        $row['HEIGHT_FORMATED'] = $rowData['HEIGHT'] . ' мм';
    }
	
		

    if ($rowData['VOLUME']){
        $row['VOLUME_FORMATED'] = $rowData['VOLUME'] . ' м3';
        $allVolume += ($rowData['VOLUME'] * $rowData['QUANTITY']);
		$allVolume = round($allVolume,2);
	...
	if (!empty($result['GRID']['HEADERS']) && is_array($result['GRID']['HEADERS']))
	{
		foreach ($result['GRID']['HEADERS'] as &$value)
		{
			...
			elseif ($value['id'] === 'LENGTH')
			{
				$value['name'] = Loc::getMessage('SBB_LENGTH_NAME');

				if (!empty($row['LENGTH_FORMATED']))
				{
					$rowData['COLUMN_LIST'][] = array(
						'CODE' => $value['id'],
						'NAME' => $value['name'],
						'VALUE' => $row['LENGTH_FORMATED'],
						'IS_TEXT' => true,
						'HIDE_MOBILE' => 'Y'
					);
				}
			}
				elseif ($value['id'] === 'HEIGHT')
			{
				$value['name'] = Loc::getMessage('SBB_HEIGHT_NAME');

				if (!empty($row['HEIGHT_FORMATED']))
				{
					$rowData['COLUMN_LIST'][] = array(
						'CODE' => $value['id'],
						'NAME' => $value['name'],
						'VALUE' => $row['HEIGHT_FORMATED'],
						'IS_TEXT' => true,
						'HIDE_MOBILE' => !isset($mobileColumns[$value['id']])
					);
				}
			}
            elseif ($value['id'] === 'WIDTH')
            {
                $value['name'] = Loc::getMessage('SBB_WIDTH_NAME');

                if (!empty($row['WIDTH_FORMATED']))
                {
                    $rowData['COLUMN_LIST'][] = array(
                        'CODE' => $value['id'],
                        'NAME' => $value['name'],
                        'VALUE' => $row['WIDTH_FORMATED'],
                        'IS_TEXT' => true,
                        'HIDE_MOBILE' => !isset($mobileColumns[$value['id']])
                    );
                }
            }
            elseif ($value['id'] === 'VOLUME')
            {
                $value['name'] = Loc::getMessage('SBB_VOLUME_NAME');

                if (!empty($row['VOLUME_FORMATED']))
                {
                    $rowData['COLUMN_LIST'][] = array(
                        'CODE' => $value['id'],
                        'NAME' => $value['name'],
                        'VALUE' => $row['VOLUME_FORMATED'],
                        'IS_TEXT' => true,
                        'HIDE_MOBILE' => !isset($mobileColumns[$value['id']])
                    );
                }
...
if ($allVolume){
    $totalData['VOLUME_FORMATED'] = $allVolume . ' м3';
}
...
{
	$totalData['HEIGHT_FORMATED'] = $result['allHeight_FORMATED'];
}
if ($result['allLength'] > 0)
{
	$totalData['LENGTH_FORMATED'] = $result['allLength_FORMATED'];
}
...
$basketD = Bitrix\Sale\Basket::loadItemsForFUser(Bitrix\Sale\Fuser::getId(), Bitrix\Main\Context::getCurrent()->getSite());
$basketItemsD = $basketD->getBasketItems();
foreach ($basketItemsD as $basketItemD) {
    $itemDim = unserialize($basketItemD->getField('DIMENSIONS'));

    foreach ($result['BASKET_ITEM_RENDER_DATA'] as &$ITEM){
        if($ITEM['ID'] == $basketItemD->getId()){
            $codes = array_column($ITEM['COLUMN_LIST'], 'CODE');
            if($itemDim['LENGTH'] && $itemDim['LENGTH']>0) {
                if (in_array('PROPERTY_DLINA_VALUE', $codes)) {
                    $index = array_search('PROPERTY_DLINA_VALUE', $codes);
                    $ITEM['COLUMN_LIST'][$index]['VALUE'] = $itemDim['LENGTH'];
                } else {
                    $ITEM['COLUMN_LIST'][] = array('CODE' => 'PROPERTY_DLINA_VALUE', 'NAME' => 'Длина, мм', 'VALUE' => $itemDim['LENGTH'], 'IS_TEXT' => true, 'HIDE_MOBILE' => true);
                }
            }
            if($itemDim['WIDTH'] && $itemDim['WIDTH']>0) {
                if (in_array('PROPERTY_IT_8_VALUE', $codes)) {
                    $index = array_search('PROPERTY_IT_8_VALUE', $codes);
                    $ITEM['COLUMN_LIST'][$index]['VALUE'] = $itemDim['WIDTH'];
                } else {
                    $ITEM['COLUMN_LIST'][] = array('CODE' => 'PROPERTY_IT_8_VALUE', 'NAME' => 'Ширина, мм', 'VALUE' => $itemDim['WIDTH'], 'IS_TEXT' => true, 'HIDE_MOBILE' => true);
                }
            }
            if($itemDim['HEIGHT'] && $itemDim['HEIGHT']>0){
                if(in_array('PROPERTY_IT_6_VALUE',$codes)){
                    $index = array_search('PROPERTY_IT_6_VALUE', $codes);
                    $ITEM['COLUMN_LIST'][$index]['VALUE'] = $itemDim['HEIGHT'];
                }else{
                    $ITEM['COLUMN_LIST'][] = array('CODE'=>'PROPERTY_IT_6_VALUE','NAME'=>'Толщина, мм','VALUE'=>$itemDim['HEIGHT'],'IS_TEXT'=>true,'HIDE_MOBILE'=>true);
                }
            }
...
foreach ($result['BASKET_ITEM_RENDER_DATA'] as &$item){
    ...
    if(in_array('PROPERTY_IT_6_VALUE', $codes) && in_array('PROPERTY_IT_8_VALUE', $codes) && in_array('PROPERTY_DLINA_VALUE', $codes)){
        ...
        $resVal = round($val*$item['QUANTITY'], 2, PHP_ROUND_HALF_EVEN).' м³';

        if($resVal == 0)$resVal = 0.01;
        $item['COLUMN_LIST'][] = array('CODE'=>'CUST_VAL','NAME'=>'Объем','IS_TEXT'=>true,'VALUE'=>$resVal,'HIDE_MOBILE'=>true);
    }
    foreach ($item['COLUMN_LIST'] as &$pr){
        if($pr['CODE'] == 'PROPERTY_IT_6_VALUE'){
            $pr['NAME'] = 'Толщина';
            if($pr['VALUE']){
                $pr['VALUE'] = $pr['VALUE'].' мм';
            }
        }
        if($pr['CODE'] == 'PROPERTY_IT_8_VALUE'){
            $pr['NAME'] = 'Ширина';
            if($pr['VALUE']){
                $pr['VALUE'] = $pr['VALUE'].' мм';
            }
        }
        if($pr['CODE'] == 'PROPERTY_DLINA_VALUE'){
            $pr['NAME'] = 'Длина';
            if($pr['VALUE']){
                $pr['VALUE'] = $pr['VALUE'].' мм';
            }
...
if($totalVal>0){
    $result['TOTAL_RENDER_DATA']['VOLUME_FORMATED'] = round($totalVal, 2).' м³';
}
