...
<div class="popup-intro">
	...
	<?$url = $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];?>
		<form method="post" onsubmit="ym(62259859,'reachGoal','SUBMITRUFORM'); return true;" name="ONECLICK" id="one_click_buy_form" action="<?=$arResult['SCRIPT_PATH']?>/script.php">
			...
			<?if($arParams["SHOW_LICENCE"] == "Y"):?>
					...
					<div class="licence_block filter label_block">
						<input type="checkbox" id="licenses_popup_OCB" name="licenses_popup_OCB" required value="Y"><label for="licenses_popup_OCB" class="license">
							<?$APPLICATION->IncludeFile(SITE_DIR."include/licenses_text_zakaz.php", Array(), Array("MODE" => "html", "NAME" => "LICENSES")); ?>
						</label>
					</div>
			...
			*/?>
			...
			<div class="licence_block filter label_block">
							...
							<label>*после отправки заказа вам перезвонит наш менеджер</label>
						</div>
			...
			<input type="hidden" name="SESSION_UTM"  data-sid="SESSION_UTM" value="<?=$utm?>" />
			<?=bitrix_sessid_post()?>
		</form>
				...
				if ($(form).find('input.error').length || $(form).find('textarea.error').length) {
					...
					if (window.renderRecaptchaById && window.asproRecaptcha && window.asproRecaptcha.key) {
						if (window.asproRecaptcha.params.recaptchaSize == 'invisible' && typeof grecaptcha != 'undefined' && arNextOptions.THEME.ONE_CLICK_BUY_CAPTCHA === 'Y') {
							...
							if (arNextOptions.THEME.ONE_CLICK_BUY_CAPTCHA === 'Y' && (window.asproRecaptcha.ver === "3" && typeof grecaptcha != "undefined")) {
								bSend = false;
				...
				$.ajax({
					...
					error: function(data) {
						oneClickResultHandler(form_url, bPopup, type, {
							result: 'N',
							message: '',
							err: '<?= GetMessage('ONE_CLICK_REQUEST_ERROR') ?>'
						});
					},
...
$(document).ready(function(){
	if($('input[data-sid="DOMENURL"]').length)
				$('input[data-sid="DOMENURL"]').val(domenurl);	
			if($('input[data-sid="URLFORM"]').length)
