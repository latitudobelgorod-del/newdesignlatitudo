	...
	<div class="flexslider">
		<ul class="slides">
			<?foreach($arResult["ITEMS"][$arParams["BANNER_TYPE_THEME"]]["ITEMS"] as $arItem):?>
				...
				?>
				<li class="box<?=($arItem["PROPERTIES"]["TEXTCOLOR"]["VALUE_XML_ID"] ? " ".$arItem["PROPERTIES"]["TEXTCOLOR"]["VALUE_XML_ID"] : "");?>
				<?=($arItem["PROPERTIES"]["TEXT_POSITION"]["VALUE_XML_ID"] ? " ".$arItem["PROPERTIES"]["TEXT_POSITION"]["VALUE_XML_ID"] : " left");?>" 
				data-nav_color="<?=($arItem["PROPERTIES"]["NAV_COLOR"]["VALUE_XML_ID"] ? $arItem["PROPERTIES"]["NAV_COLOR"]["VALUE_XML_ID"] : "");?>" 
				id="<?=$this->GetEditAreaId($arItem['ID']);?>" style="background-image: url('<?=$background?>');">
		...
		$arSelect = Array("ID", "NAME", "PROPERTIES_LINK_VIDEO");
		$res = CIBlockElement::GetList(Array("ID"=>"ASC"), Array("ID"=>$arItem['PROPERTIES']['LINK_YOUTUBE']['VALUE'], "IBLOCK_ID" => 23, ), false, false, Array("NAME", "PROPERTY_LINK_VIDEO", "PREVIEW_PICTURE") );
		$home = array();
		...
		$res_images = CFile::ResizeImageGet($vid["PREVIEW_PICTURE"], Array("width" => 500, "height" => 280), BX_RESIZE_IMAGE_EXACT, false);
    	if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $url, $match)) {
		$video_id = $match[1];
		}
		?>
						<div class="size_video m_slides" >
						<a href='https://www.youtube.com/embed/<?=$video_id?>'  class="gallery" rel="group">
       	   				<div class="preview_pic"><img src="<?=$res_images["src"]?>"  class="img-responsive" /></div>
						</a>
												<?if($arItem["NAME"]):?>
												<div class="banner_nametext">
											...
											<?if($arItem["NAME"]):?>
												<div class="banner_title">
													<?if($arItem["PROPERTIES"]["METKA"]["VALUE"]):?>
														<span class="metka"><?=$arItem["PROPERTIES"]["METKA"]["VALUE"];?></span>
													<? endif; ?>
													<span>
															<?if($arItem["PROPERTIES"]["URL_STRING"]["VALUE"]):?>
															<a href="<?=$arItem["PROPERTIES"]["URL_STRING"]["VALUE"]?>" <?=(strlen($target) ? 'target="'.$target.'"' : '')?>>
