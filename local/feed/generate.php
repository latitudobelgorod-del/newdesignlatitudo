<?php
/**
 * НЕ ИСПОЛЬЗУЕТСЯ (с 10 сентября 2026). Полные фиды ведёт модуль
 * «Маркет для продавцов» — прайс-листы «Полный товарный фид <город>»
 * (группа «Полный товарный фид», файлы /bitrix/catalog_export/*_full.xml):
 * Ирина хочет видеть и править их в админке. Карточку предложения там
 * наполняет обработчик local/php_interface/include/latitudo_market_feed.php,
 * создавались прайс-листы скриптом local/tools/nd_market_full_feeds.php.
 * Этот генератор снят с cron, его файлы удалены; оставлен как запасной
 * вариант и как эталон состава и цен (сверка с модулем: 0 расхождений).
 *
...
foreach ($items as $it) {
    ...
    // Цена в фид идёт за штуку — ровно та, что покупатель платит за одну
    // позицию (строка «Общая стоимость» на карточке). До 16 сентября 2026
    // здесь домножали на множитель основной единицы (BASE_KOEF), чтобы фид
    // совпал с крупной ценой «3 243 ₽/м²». Служба контроля качества Яндекс
    // Товаров заблокировала за это источник: со страницы она читает цену за
    // штуку, а цену за м² в фиде считает расхождением. Поля KOEFS и BASE_UNIT
    // собираются выше по-прежнему — для тегов, не для цены.
