...
//    echo '<pre>', print_r($utm), '</pre>' ;
   ...
   $arResult["QUESTIONS"]['UTM_MEDIUM']['VALUE'] = $utm_medium;
	...
	if($orderID){
		...
		$arOfferTreeProperties = $arOfferProperties = $arPropsSku = [];
		if (strlen($_REQUEST['OFFER_TREE_PROPERTIES'])) {
			$arOfferTreeProperties = json_decode($_REQUEST["OFFER_TREE_PROPERTIES"]);
		}

		if (strlen($_REQUEST['OFFER_PROPERTIES'])) {
			$arOfferProperties = json_decode($_REQUEST["OFFER_PROPERTIES"]);
		}

		$arPropsSku = array_unique(array_merge((array)$arOfferTreeProperties, (array)$arOfferProperties));
		...
		$arProductsList = \CCatalogSKU::getProductList($arProductsIds);
		$arSkuIds = array_keys($arProductsList);
		
		if (
			$arSkuIds &&
			is_array($arProductsList) &&
			$arPropsSku
		) {
			$skuIblockId = reset($arProductsList)['OFFER_IBLOCK_ID'];

			$arSkuPropertiesText = $arHlBlocksEntityClassByTableName = [];
			$result = [];
			CIBlockElement::GetPropertyValuesArray($result, $skuIblockId, ['ID' => $arSkuIds], ['CODE' => $arPropsSku]);
			foreach ($result as $skuId => $arSkuProperties) {
				$skuPropertiesValuesText = '';
				foreach ($arSkuProperties as $propCode => $arSkuProperty) {
					$arFieldsElement = CIBlockFormatProperties::GetDisplayValue([], $arSkuProperty);
					if ($arFieldsElement['DISPLAY_VALUE'] || strlen($arFieldsElement['DISPLAY_VALUE'])) {
						$skuPropertiesValuesText .= $arFieldsElement['NAME'].': '. $arFieldsElement['DISPLAY_VALUE']. '; ';
					}
		...
		$arMessageFields = array(
			...
			"DOMENURL" => $_POST['DOMENURL'],
			"SESSION_UTM" => $_POST["SESSION_UTM"],
	...
	$phonecopy = $_POST["ONE_CLICK_BUY"]["PHONE"];
$strWithoutChars = preg_replace('/[^0-9]/', '', $phonecopy );
$summacopy = $arMessageFields["ORDER_PRICE"];
$strsummacopy = str_replace(' ', '', $summacopy);
$strsummacopy1 = str_replace('руб', '', $strsummacopy); 
$pagecopy = $arMessageFields["URLFORM"];
$commentcopy = $_POST['ONE_CLICK_BUY']['COMMENT'];
$namecopy = $arMessageFields["CLIENT_NAME"];
$domencopy = $arMessageFields["DOMENURL"];
$addresscopy = $arMessageFields["CONTACT_ADDRESS"];
$emailcopy = $arMessageFields["EMAIL_BUYER"];
$commentscopy = $arMessageFields["ORDER_ITEMS"];
$utm_source = $arMessageFields["UTM_SOURCE"];
$utm_campaign = $arMessageFields["UTM_CAMPAIGN"];
$utm_content = $arMessageFields["UTM_CONTENT"];
...
$ParamLid = http_build_query(array(
  'fields' => array(
   ...
   'NAME' => $namecopy, // ИМЯ
	...
	'UF_CRM_1670914240846' => $pagecopy, // форма заполнена на странице, страница заявки
	'UF_CRM_1674473471' => $utm_source, //utm-source
	'UF_CRM_1674473492' => $utm_campaign,//utm-campaign
	'UF_CRM_1674473502' => $utm_content,// utm-content
	'UF_CRM_1674473509' => $utm_term, //utm-term
